				...
				@if($note->completed == 0)
					...
					<div class="max-w-sm rounded overflow-hidden w-full shadow-lg bg-gray-100 my-5">
						...
						<div class="px-6 pt-4 pb-2 flex">
							<span class="inline-block bg-blue-500 rounded-full px-3 py-1 text-sm font-semibold text-black-700 mr-2 mb-2">
								<a class="text-center" href="{{ route('notes.show', $note) }}">
									Actualizar Nota
								</a>
							</span>
							<span class="inline-block bg-red-500 rounded-full px-3 py-1 text-sm font-semibold text-black-700 mr-2 mb-2">
								<form action="{{ route('notes.destroy', $note) }}" method="POST">

								@csrf
								@method('delete')
								<button class="btn btn-danger" type="submit"> Borra Nota </button>
							</form>
						</span>
					</div>
					<div class="p-2 mx-5">
